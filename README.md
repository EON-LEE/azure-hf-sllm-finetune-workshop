...
이 리포의 숫자는 전부 라이브 Azure 구독에서 잰 것입니다.
**전체 로드테스트 결과·그래프·원자료는 [`docs/PERFORMANCE.md`](docs/PERFORMANCE.md)** 에 있습니다.

| 항목 | 실측값 | 근거 |
|---|---|---|
| 27B QLoRA 학습 (30스텝, seq 1024, 유효배치 16) | **41.6분**, `train_loss 1.2637` | [PERFORMANCE §9](docs/PERFORMANCE.md) |
| 학습 VRAM 피크 | **28.19 GB** → 40 GB GPU 필요 | [PERFORMANCE §9](docs/PERFORMANCE.md) |
| 학습 파라미터 | 116.73M (**0.79%**) | [PERFORMANCE §9](docs/PERFORMANCE.md) |
| 관리형 엔드포인트 서빙 (A100 1대, thinking OFF) | **peak 189 tok/s**, knee=16, p95 TTFT 1.32초 | [PERFORMANCE §2](docs/PERFORMANCE.md) |
| 같은 부하, 베이스 모델 대조군 | peak 204 tok/s, **TPOT 는 동일** (0.0407초) | [PERFORMANCE §4](docs/PERFORMANCE.md) |
| tok/s 격차 223토큰의 출처 | **프롬프트 1개** — 나머지는 `max_tokens` 상한에 눌림 | [PERFORMANCE §6](docs/PERFORMANCE.md) |
| 로드테스트 성공률 | **200/200, 실패 0** (2배포 × 5레벨 × 20요청) | [PERFORMANCE §2](docs/PERFORMANCE.md) |
| 출력 토큰 원가 (knee 기준) | **$7.29 / 100만 토큰** | [PERFORMANCE §7](docs/PERFORMANCE.md) |
| 사고 토큰 스트림 | SSE 4921 프레임 중 4920 이 `reasoning` 필드 | [PERFORMANCE §8](docs/PERFORMANCE.md) |
| 3개 게이트 (학습 → 배포 → 로드테스트) | 전부 통과 | [PERFORMANCE §10](docs/PERFORMANCE.md) |

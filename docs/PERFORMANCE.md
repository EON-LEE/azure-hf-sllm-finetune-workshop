...
| 축 | 값 |
|---|---|
| 리전 | **polandcentral** — 이 회차가 실제로 돈 곳 (koreacentral 은 A100 존 제한, [GOTCHAS §3](GOTCHAS.md)) |
...
>   (Retail Prices API · koreacentral · Consumption), LowPriority $0.992 는 같은 API의
>   표(koreacentral · Linux · Consumption, 2026-08-27 조회)에서 왔습니다. 두 값 다
...
>   `type eq 'Consumption'` 로 조회해 위 두 값과 대조하세요. 행 고르기 함정 셋
>   (Reservation·DevTest·`Linux` 문자열)을 주의하세요. 조회 전까지 이 문단을 지우지
>   마세요.

> ### ⚠️ $0.992/시 는 **요율**이고, 잡 1회 총액과 혼동하지 마세요
> **잡 1회 총액을 요율로 착각하기 쉽습니다.** [Lab 2](labs/lab2.md) 가 적어 둔 것도
> "잡 1회 실측이지 요율이 아닙니다" 입니다.
>
> - **요율 $0.992/시** — `Standard_NC24ads_A100_v4` LowPriority. Azure Retail Prices
>   API, koreacentral, Linux, Consumption, 2026-08-27 조회. PAYG $4.959 의 0.20 배.
> - **요율만으로 잡 총액을 되돌리지 마세요.** 학습 구간 42.3분에 요율을 곱하면
>   **$0.70**인데, 잡 하나의 실측 총액은 이보다 큽니다 — 그 잡은 학습만 한 게
>   아니라 54 GB 다운로드와 27B 2회 적재·채점을 같은 노드에서 했고, **잡 전체의
>   노드 점유 시간이 어디에도 기록돼 있지 않습니다.** 42.3분은 학습 구간의
>   벽시계일 뿐입니다.
> - 그래서 **총액 ÷ 학습 구간 시간으로 시간당 요율을 역산하지 마세요.** 그건
>   요율이 아니라 잡 하나의 오버헤드까지 요율로 착각한 값입니다.
> - 이 요율은 **학습 클러스터에만** 씁니다. 관리형 온라인 엔드포인트는 LowPriority 를
>   쓸 수 없으므로 서빙 줄은 PAYG $4.959 그대로입니다.
...
`<think>\n\n</think>` 를 미리 채워 넣으므로 사고 블록이 생성되지 않습니다.
`chat_template_kwargs` 를 안 보낸 조건(템플릿 기본값, 사고 ON)에서 blue 의 사고과정
누출을 원인으로 지목한 적이 있는데, **조건이 다른 측정을 옮겨 적은 오류**였습니다.
...
같은 요청이 1.04초로도, 12.94초로도 기록됩니다.**

- **사고 델타의 필드명은 `reasoning` 입니다.** `reasoning_content` 가 아닙니다.
  이 오타 하나로 "thinking 이 고장났다" 는 잘못된 결론을 한 번 냈습니다.
...
| 구간 | 실측 |
|---|---|
| 27B QLoRA 학습 (30스텝, seq 1024, 유효배치 16) | **41.6분**, `train_loss 1.2637` |
| 학습 VRAM 피크 | **28.19 GB** → 40GB 이상 GPU 필요 |
| 학습 파라미터 | 116.73M (전체의 **0.79%**) |
| LoRA 병합 잡 | **12분 53초** |
| 병합 가중치 blob → 컨테이너 수신 | 21 blob **50.1 GiB 를 111초** |
| 배포 준비 완료 (`Succeeded`) | **24분** |
| 배포 readiness 예산 | `PT2M + 45×PT15S` = **13.25분** |
...
`ffsft.serve.smoke` 가 본문을 읽기 때문에 잡혔습니다.
...
`1.564` 가 **서로 다른 값**이었습니다. SVG 는 손으로 만들지 않았고 JSON 에서
...
각 Lab 의 "기대 출력" 은 **이 문서에서 잘라 갑니다.** `status` 명령이 실제로 찍는
화면을 아래에 그대로 남깁니다.
...
머릿속으로 돌리면 `lowpriority` (밑줄 없이) 가 나오는데, 이것은 **틀린 철자**입니다 —
Lab 문서에 옮겨 적을 때 이 밑줄을 빠뜨리지 마세요.
